...
object Calculator {
  def computeValues(
    ...
    namedExpressions.map(expression => (expression._1, Signal(eval(expression._2(), namedExpressions))))
  }
  ...
  def eval(expr: Expr, references: Map[String, Signal[Expr]]): Double = {
    ...
    expr match {
      ...
      case Ref(name) =>
        ...
        else eval(getReferenceExpr(name, references), references.filterKeys(_ != name))
